// date-fns-tz/format/formatters/index.ts
...
const formatters = {
  ...
  X(date: Date, token: string, _localize: boolean | null, options: { timeZone: string; _originalDate?: Date }): string {
  ...
  // Timezone (GMT)
  O(date: Date, token: string, _localize: boolean | null, options: { timeZone: string; _originalDate?: Date }): string {
    const timezoneOffset = getTimeZoneOffset(options.timeZone, options._originalDate || date);
...
function addLeadingZeros(number_: number, targetLength: number) {
  const sign = number_ < 0 ? '-' : '';
  let output = Math.abs(number_).toString();
