...
describe('differenceInWeeks', () => {
  it('returns the number of full weeks between the given dates', () => {
    const result = differenceInWeeks(new Date(2014, 6 /* Jul */, 8, 18, 0), new Date(2014, 5 /* Jun */, 29, 6, 0));
    expect(result).toBe(1);
  });

  it('returns the number of weeks between the given dates with `trunc` as default a rounding method', () => {
    const result = differenceInWeeks(new Date(2014, 5 /* Jun */, 29, 6, 0), new Date(2014, 6 /* Jul */, 13, 5, 0));
    expect(result).toBe(-1);
  });
  ...
  it('returns the number of weeks between the given dates with `round` passed in as a rounding method', () => {
    const result = differenceInWeeks(new Date(2014, 6 /* Jul */, 10, 18, 0), new Date(2014, 5 /* Jun */, 29, 6, 0), {
  ...
  it('returns a negative number if the time value of the first date is smaller', () => {
    const result = differenceInWeeks(new Date(2014, 5 /* Jun */, 29, 6, 0), new Date(2014, 6 /* Jul */, 8, 18, 0));
    expect(result).toBe(-1);
  });
  ...
  describe('edge cases', () => {
    it('the difference is less than a week, but the given dates are in different calendar weeks', () => {
      const result = differenceInWeeks(new Date(2014, 6 /* Jul */, 6), new Date(2014, 6 /* Jul */, 5));
      expect(result).toBe(0);
    });

    it('the same for the swapped dates', () => {
      const result = differenceInWeeks(new Date(2014, 6 /* Jul */, 5), new Date(2014, 6 /* Jul */, 6));
      expect(result).toBe(0);
    });

    it('days of weeks of the given dates are the same', () => {
      const result = differenceInWeeks(new Date(2014, 6 /* Jul */, 9), new Date(2014, 6 /* Jul */, 2));
      expect(result).toBe(1);
    });

    it('the given dates are the same', () => {
      const result = differenceInWeeks(new Date(2014, 8 /* Sep */, 5, 0, 0), new Date(2014, 8 /* Sep */, 5, 0, 0));
      expect(result).toBe(0);
    });
  ...
  it('returns NaN if the first date is `Invalid Date`', () => {
    const result = differenceInWeeks(new Date(NaN), new Date(2017, 0 /* Jan */, 1));
    expect(isNaN(result)).toBe(true);
  });

  it('returns NaN if the second date is `Invalid Date`', () => {
    const result = differenceInWeeks(new Date(2017, 0 /* Jan */, 1), new Date(NaN));
    expect(isNaN(result)).toBe(true);
  });
