...
describe('subMilliseconds', () => {
  it('subtracts the given number of milliseconds', () => {
    const result = subMilliseconds(new Date(2014, 6 /* Jul */, 10, 12, 45, 30, 0), 750);
    expect(result).toEqual(new Date(2014, 6 /* Jul */, 10, 12, 45, 29, 250));
  });

  it('accepts a timestamp', () => {
    const result = subMilliseconds(new Date(2014, 6 /* Jul */, 10, 12, 45, 30, 0).getTime(), 500);
  ...
  it('returns `Invalid Date` if the given amount is NaN', () => {
    const result = subMilliseconds(new Date(2014, 6 /* Jul */, 10, 12, 45, 30, 0), NaN);
    expect(result instanceof Date && isNaN(result.getTime())).toBe(true);
  });
});
