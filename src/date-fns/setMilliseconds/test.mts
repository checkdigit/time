...
describe('setMilliseconds', () => {
  it('sets the milliseconds', () => {
    const result = setMilliseconds(new Date(2014, 8 /* Sep */, 1, 11, 30, 40, 500), 300);
    expect(result).toEqual(new Date(2014, 8 /* Sep */, 1, 11, 30, 40, 300));
  });

  it('accepts a timestamp', () => {
    const result = setMilliseconds(new Date(2014, 8 /* Sep */, 1, 11, 30, 15, 750).getTime(), 755);
  ...
  it('returns `Invalid Date` if the given amount is NaN', () => {
    const result = setMilliseconds(new Date(2014, 8 /* Sep */, 1, 11, 30, 40, 500), NaN);
    expect(result instanceof Date && isNaN(result.getTime())).toBe(true);
  });
});
