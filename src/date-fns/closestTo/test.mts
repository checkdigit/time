...
describe('closestTo', () => {
  it('returns the date from the given array closest to the given date', () => {
    ...
    const result = closestTo(date, [new Date(2015, 7 /* Aug */, 31), new Date(2012, 6 /* Jul */, 2)]);
  ...
  it('returns `Invalid Date` if any date in the given array is `Invalid Date`', () => {
    const date = new Date(2014, 6 /* Jul */, 2);
    const result = closestTo(date, [new Date(2015, 7 /* Aug */, 31), new Date(NaN), new Date(2012, 6 /* Jul */, 2)]);
