...
describe('parse', () => {
  ...
  it('escapes characters between the single quote characters', () => {
    const result = parse('2018 hello world July 2nd', "yyyy 'hello world' MMMM do", referenceDate);
    ...
    describe('validation', () => {
      [
        ...
        it(`throws an error when G is used after ${token}`, () => {
          const block = () => parse(`${example} 420`, `${token} G`, referenceDate);
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`G\` at the same time`),
          );
        });
      });
    });
    ...
    describe('validation', () => {
      [
        ...
        it(`throws an error when y is used after ${token}`, () => {
          const block = () => parse(`${example} 2019`, `${token} y`, referenceDate);
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`y\` at the same time`),
          );
        });
      });
    });
  ...
  describe('local week-numbering year', () => {
    ...
    describe('validation', () => {
      const tokensToValidate: Array<[string, string, { useAdditionalDayOfYearTokens: boolean }?]> = [
        ['y', '2019'],
        ['Y', '2019'],
        ['R', '2019'],
        ['u', '2019'],
      ...
      tokensToValidate.forEach(([token, example, options]) => {
        it(`throws an error when Y is used after ${token}`, () => {
          const block = () => parse(`${example} 2019`, `${token} Y`, referenceDate, options);
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`Y\` at the same time`),
          );
    ...
    describe('validation', () => {
      ...
      tokensToValidate.forEach(([token, example, options]) => {
        it(`throws an error when R is used after ${token}`, () => {
          const block = () => parse(`${example} 2019`, `${token} R`, referenceDate, options);
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`R\` at the same time`),
          );
        });
      });
    });
    ...
    describe('validation', () => {
      [
        ...
        it(`throws an error when u is used after ${token}`, () => {
          const block = () => parse(`${example} 2019`, `${token} u`, referenceDate);
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`u\` at the same time`),
          );
        });
      });
    });
    ...
    describe('validation', () => {
      ...
      tokensToValidate.forEach(([token, example, options]) => {
        it(`throws an error when Q is used after ${token}`, () => {
          const block = () => parse(`${example} 1`, `${token} Q`, referenceDate, options);
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`Q\` at the same time`),
          );
        });
      });
    });
    ...
    describe('validation', () => {
      ...
      tokensToValidate.forEach(([token, example, options]) => {
        it(`throws an error when M is used after ${token}`, () => {
          const block = () => parse(`${example} 1`, `${token} M`, referenceDate, options);
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`M\` at the same time`),
          );
        });
      });
    });
    ...
    describe('validation', () => {
      ...
      tokensToValidate.forEach(([token, example, options]) => {
        it(`throws an error when L is used after ${token}`, () => {
          const block = () => parse(`${example} 1`, `${token} L`, referenceDate, options);
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`L\` at the same time`),
          );
        });
      });
    });
    ...
    describe('validation', () => {
      ...
      tokensToValidate.forEach(([token, example, options]) => {
        it(`throws an error when w is used after ${token}`, () => {
          const block = () => parse(`${example} 1`, `${token} w`, referenceDate, options);
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`w\` at the same time`),
          );
        });
      });
    });
    ...
    describe('validation', () => {
      ...
      tokensToValidate.forEach(([token, example, options]) => {
        it(`throws an error when I is used after ${token}`, () => {
          const block = () => parse(`${example} 1`, `${token} I`, referenceDate, options);
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`I\` at the same time`),
          );
        });
      });
    });
    ...
    describe('validation', () => {
      ...
      tokensToValidate.forEach(([token, example, options]) => {
        it(`throws an error when d is used after ${token}`, () => {
          const block = () => parse(`${example} 1`, `${token} d`, referenceDate, options);
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`d\` at the same time`),
          );
        });
      });
    });
    ...
    describe('validation', () => {
      ...
      tokensToValidate.forEach(([token, example, options]) => {
        it(`throws an error when E is used after ${token}`, () => {
          const block = () => parse(`${example} Mon`, `${token} E`, referenceDate, options);
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`E\` at the same time`),
          );
        });
      });
    });
  ...
  describe('local day of week (formatting)', () => {
    ...
    describe('validation', () => {
      ...
      tokensToValidate.forEach(([token, example, options]) => {
        it(`throws an error when e is used after ${token}`, () => {
          const block = () => parse(`${example} 1`, `${token} e`, referenceDate, options);
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`e\` at the same time`),
          );
    ...
    describe('validation', () => {
      ...
      tokensToValidate.forEach(([token, example, options]) => {
        it(`throws an error when c is used after ${token}`, () => {
          const block = () => parse(`${example} 1`, `${token} c`, referenceDate, options);
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`c\` at the same time`),
          );
        });
      });
    });
    ...
    describe('validation', () => {
      [
        ...
        it(`throws an error when a is used after ${token}`, () => {
          const block = () => parse(`${example} AM`, `${token} a`, referenceDate);
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`a\` at the same time`),
          );
        });
      });
    });
    ...
    describe('validation', () => {
      [
        ...
        it(`throws an error when b is used after ${token}`, () => {
          const block = () => parse(`${example} AM`, `${token} b`, referenceDate);
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`b\` at the same time`),
          );
        });
      });
    });
  ...
  describe('flexible day period', () => {
    ...
    describe('validation', () => {
      [
        ...
        it(`throws an error when B is used after ${token}`, () => {
          const block = () => parse(`${example} in the morning`, `${token} B`, referenceDate);
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`B\` at the same time`),
          );
    ...
    describe('validation', () => {
      [
        ...
        it(`throws an error when h is used after ${token}`, () => {
          ...
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`h\` at the same time`),
          );
        });
      });
    });
    ...
    describe('validation', () => {
      [
        ...
        it(`throws an error when K is used after ${token}`, () => {
          ...
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`K\` at the same time`),
          );
        });
      });
    });
    ...
    describe('validation', () => {
      [
        ...
        it(`throws an error when s is used after ${token}`, () => {
          ...
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`s\` at the same time`),
          );
        });
      });
    });
  ...
  describe('timezone (ISO-8601 w/ Z)', () => {
    describe('X', () => {
      it('hours and minutes', () => {
        const result = parse('2016-11-25T16:38:38.123-0530', "yyyy-MM-dd'T'HH:mm:ss.SSSX", referenceDate);
        expect(result).toEqual(new Date('2016-11-25T16:38:38.123-05:30'));
      });

      it('GMT', () => {
        const result = parse('2016-11-25T16:38:38.123Z', "yyyy-MM-dd'T'HH:mm:ss.SSSX", referenceDate);
        expect(result).toEqual(new Date('2016-11-25T16:38:38.123Z'));
      });

      it('hours', () => {
        const result = parse('2016-11-25T16:38:38.123+05', "yyyy-MM-dd'T'HH:mm:ss.SSSX", referenceDate);
    ...
    describe('XX', () => {
      it('hours and minutes', () => {
        const result = parse('2016-11-25T16:38:38.123-0530', "yyyy-MM-dd'T'HH:mm:ss.SSSXX", referenceDate);
        expect(result).toEqual(new Date('2016-11-25T16:38:38.123-05:30'));
      });

      it('GMT', () => {
        const result = parse('2016-11-25T16:38:38.123Z', "yyyy-MM-dd'T'HH:mm:ss.SSSXX", referenceDate);
        expect(result).toEqual(new Date('2016-11-25T16:38:38.123Z'));
      });
    });
    ...
    describe('XXX', () => {
      it('hours and minutes', () => {
        const result = parse('2016-11-25T16:38:38.123-05:30', "yyyy-MM-dd'T'HH:mm:ss.SSSXXX", referenceDate);
        expect(result).toEqual(new Date('2016-11-25T16:38:38.123-05:30'));
      });

      it('GMT', () => {
        const result = parse('2016-11-25T16:38:38.123Z', "yyyy-MM-dd'T'HH:mm:ss.SSSXXX", referenceDate);
        expect(result).toEqual(new Date('2016-11-25T16:38:38.123Z'));
      });
    });
    ...
    describe('XXXX', () => {
      it('hours and minutes', () => {
        const result = parse('2016-11-25T16:38:38.123-0530', "yyyy-MM-dd'T'HH:mm:ss.SSSXXXX", referenceDate);
        expect(result).toEqual(new Date('2016-11-25T16:38:38.123-05:30'));
      });

      it('GMT', () => {
        const result = parse('2016-11-25T16:38:38.123Z', "yyyy-MM-dd'T'HH:mm:ss.SSSXXXX", referenceDate);
        expect(result).toEqual(new Date('2016-11-25T16:38:38.123Z'));
      });

      it('hours, minutes and seconds', () => {
        const result = parse('2016-11-25T16:38:38.123+053045', "yyyy-MM-dd'T'HH:mm:ss.SSSXXXX", referenceDate);
    ...
    describe('XXXXX', () => {
      it('hours and minutes', () => {
        const result = parse('2016-11-25T16:38:38.123-05:30', "yyyy-MM-dd'T'HH:mm:ss.SSSXXXXX", referenceDate);
        expect(result).toEqual(new Date('2016-11-25T16:38:38.123-05:30'));
      });

      it('GMT', () => {
        const result = parse('2016-11-25T16:38:38.123Z', "yyyy-MM-dd'T'HH:mm:ss.SSSXXXXX", referenceDate);
        expect(result).toEqual(new Date('2016-11-25T16:38:38.123Z'));
      });

      it('hours, minutes and seconds', () => {
        const result = parse('2016-11-25T16:38:38.123+05:30:45', "yyyy-MM-dd'T'HH:mm:ss.SSSXXXXX", referenceDate);
        expect(result).toEqual(new Date('2016-11-25T16:37:53.123+05:30'));
      });
    });
    ...
    describe('validation', () => {
      [
        ...
        it(`throws an error when X is used after ${token}`, () => {
          const block = () => parse(`${example} -0530`, `${token} X`, referenceDate);
          expect(block).toThrow(RangeError);
          expect(block).toThrow(
            new RegExp(`The format string mustn't contain \`${token}\` and \`X\` at the same time`),
          );
        });
      });
    });
  ...
  describe('timezone (ISO-8601 w/o Z)', () => {
    describe('x', () => {
      ...
      it('GMT', () => {
        const result = parse('2016-11-25T16:38:38.123+0000', "yyyy-MM-dd'T'HH:mm:ss.SSSx", referenceDate);
        expect(result).toEqual(new Date('2016-11-25T16:38:38.123Z'));
      });
    ...
    describe('xx', () => {
      ...
      it('GMT', () => {
        const result = parse('2016-11-25T16:38:38.123+0000', "yyyy-MM-dd'T'HH:mm:ss.SSSxx", referenceDate);
        expect(result).toEqual(new Date('2016-11-25T16:38:38.123Z'));
      });
    });
    ...
    describe('xxx', () => {
      ...
      it('GMT', () => {
        const result = parse('2016-11-25T16:38:38.123+00:00', "yyyy-MM-dd'T'HH:mm:ss.SSSxxx", referenceDate);
        expect(result).toEqual(new Date('2016-11-25T16:38:38.123Z'));
      });
    });
    ...
    describe('xxxx', () => {
      ...
      it('GMT', () => {
        const result = parse('2016-11-25T16:38:38.123+0000', "yyyy-MM-dd'T'HH:mm:ss.SSSxxxx", referenceDate);
        expect(result).toEqual(new Date('2016-11-25T16:38:38.123Z'));
      });
    ...
    describe('xxxxx', () => {
      ...
      it('GMT', () => {
        const result = parse('2016-11-25T16:38:38.123+00:00', "yyyy-MM-dd'T'HH:mm:ss.SSSxxxxx", referenceDate);
        expect(result).toEqual(new Date('2016-11-25T16:38:38.123Z'));
      });
  ...
  describe('seconds timestamp', () => {
    ...
    it('specified amount of digits', () => {
      const result = parse('00000000000512969520', 'tttttttttttttttttttt', referenceDate);
      expect(result).toEqual(new Date(512969520000));
    });
    ...
    it(`throws an error when it is used after any token`, () => {
      ...
      expect(block).toThrow(RangeError);
      expect(block).toThrow(new RegExp(`The format string mustn't contain \`t\` and any other token at the same time`));
    });
  });
  ...
  describe('milliseconds timestamp', () => {
    ...
    it('specified amount of digits', () => {
      const result = parse('00000000512969520900', 'TTTTTTTTTTTTTTTTTTTT', referenceDate);
      expect(result).toEqual(new Date(512969520900));
    });
  ...
  describe('common formats', () => {
    it('ISO-8601', () => {
      const result = parse('20161105T040404', "yyyyMMdd'T'HHmmss", referenceDate);
      expect(result).toEqual(new Date(2016, 10 /* Nov */, 5, 4, 4, 4, 0));
    });

    it('ISO week-numbering date', () => {
      const result = parse('2016W474T153005', "RRRR'W'IIi'T'HHmmss", referenceDate);
    ...
    it('middle-endian', () => {
      const result = parse('5 a.m. 07/02/2016', 'h aaaa MM/dd/yyyy', referenceDate);
  ...
  describe('useAdditionalWeekYearTokens and useAdditionalDayOfYearTokens options', () => {
    it('throws an error if D token is used', () => {
      try {
        ...
        expect(e instanceof RangeError && e.message.startsWith('Use `d` instead of `D`')).toBe(true);
      }
    });
    ...
    it('throws an error if DD token is used', () => {
      try {
        ...
        expect(e instanceof RangeError && e.message.startsWith('Use `dd` instead of `DD`')).toBe(true);
      }
    });
    ...
    it('throws an error if YY token is used', () => {
      try {
        ...
        expect(e instanceof RangeError && e.message.startsWith('Use `yy` instead of `YY`')).toBe(true);
      }
    });
    ...
    it('throws an error if YYYY token is used', () => {
      try {
        ...
        expect(e instanceof RangeError && e.message.startsWith('Use `yyyy` instead of `YYYY`')).toBe(true);
      }
    });
