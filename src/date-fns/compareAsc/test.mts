...
describe('compareAsc', () => {
  it('returns 0 if the given dates are equal', () => {
    const result = compareAsc(new Date(1989, 6 /* Jul */, 10), new Date(1989, 6 /* Jul */, 10));
    expect(result).toBe(0);
  });

  it('returns -1 if the first date is before the second one', () => {
    const result = compareAsc(new Date(1987, 1 /* Feb */, 11), new Date(1989, 6 /* Jul */, 10));
    expect(result).toBe(-1);
  });

  it('returns 1 if the first date is after the second one', () => {
    const result = compareAsc(new Date(1989, 6 /* Jul */, 10), new Date(1987, 1 /* Feb */, 11));
    expect(result).toBe(1);
  });
  ...
  it('accepts timestamps', () => {
    const result = compareAsc(new Date(1987, 1 /* Feb */, 11).getTime(), new Date(1989, 6 /* Jul */, 10).getTime());
    expect(result).toBe(-1);
  });
