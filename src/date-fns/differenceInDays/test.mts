...
describe('differenceInDays', () => {
  it('returns the number of full days between the given dates', () => {
    const result = differenceInDays(new Date(2012, 6 /* Jul */, 2, 18, 0), new Date(2011, 6 /* Jul */, 2, 6, 0));
    expect(result).toBe(366);
  });

  it('returns a negative number if the time value of the first date is smaller', () => {
    const result = differenceInDays(new Date(2011, 6 /* Jul */, 2, 6, 0), new Date(2012, 6 /* Jul */, 2, 18, 0));
    expect(result).toBe(-366);
  });
  ...
  describe('edge cases', () => {
    it('the difference is less than a day, but the given dates are in different calendar days', () => {
      const result = differenceInDays(new Date(2014, 8 /* Sep */, 5, 0, 0), new Date(2014, 8 /* Sep */, 4, 23, 59));
      expect(result).toBe(0);
    });

    it('the same for the swapped dates', () => {
      const result = differenceInDays(new Date(2014, 8 /* Sep */, 4, 23, 59), new Date(2014, 8 /* Sep */, 5, 0, 0));
      expect(result).toBe(0);
    });

    it('the time values of the given dates are the same', () => {
      const result = differenceInDays(new Date(2014, 8 /* Sep */, 6, 0, 0), new Date(2014, 8 /* Sep */, 5, 0, 0));
      expect(result).toBe(1);
    });

    it('the given dates are the same', () => {
      const result = differenceInDays(new Date(2014, 8 /* Sep */, 5, 0, 0), new Date(2014, 8 /* Sep */, 5, 0, 0));
      expect(result).toBe(0);
    });
  ...
  it('returns NaN if the first date is `Invalid Date`', () => {
    const result = differenceInDays(new Date(NaN), new Date(2017, 0 /* Jan */, 1));
    expect(isNaN(result)).toBe(true);
  });

  it('returns NaN if the second date is `Invalid Date`', () => {
    const result = differenceInDays(new Date(2017, 0 /* Jan */, 1), new Date(NaN));
    expect(isNaN(result)).toBe(true);
  });
