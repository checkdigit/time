...
describe('differenceInMilliseconds', () => {
  ...
  it('does not return -0 when the given dates are the same', () => {
    ...
    const result = differenceInMilliseconds(new Date(2014, 8 /* Sep */, 5, 0, 0), new Date(2014, 8 /* Sep */, 5, 0, 0));
  ...
  it('returns NaN if the first date is `Invalid Date`', () => {
    const result = differenceInMilliseconds(new Date(NaN), new Date(2017, 0 /* Jan */, 1));
    expect(isNaN(result)).toBe(true);
  });

  it('returns NaN if the second date is `Invalid Date`', () => {
    const result = differenceInMilliseconds(new Date(2017, 0 /* Jan */, 1), new Date(NaN));
    expect(isNaN(result)).toBe(true);
  });
