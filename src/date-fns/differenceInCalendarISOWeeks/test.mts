...
describe('differenceInCalendarISOWeeks', () => {
  ...
  describe('edge cases', () => {
    it('the difference is less than an ISO week, but the given dates are in different calendar ISO weeks', () => {
      const result = differenceInCalendarISOWeeks(new Date(2014, 6 /* Jul */, 7), new Date(2014, 6 /* Jul */, 6));
      expect(result).toBe(1);
    });

    it('the same for the swapped dates', () => {
      const result = differenceInCalendarISOWeeks(new Date(2014, 6 /* Jul */, 6), new Date(2014, 6 /* Jul */, 7));
      expect(result).toBe(-1);
    });

    it('the days of weeks of the given dates are the same', () => {
      const result = differenceInCalendarISOWeeks(new Date(2014, 6 /* Jul */, 9), new Date(2014, 6 /* Jul */, 2));
      expect(result).toBe(1);
    });
  ...
  it('returns NaN if the first date is `Invalid Date`', () => {
    const result = differenceInCalendarISOWeeks(new Date(NaN), new Date(2017, 0 /* Jan */, 1));
    expect(isNaN(result)).toBe(true);
  });

  it('returns NaN if the second date is `Invalid Date`', () => {
    const result = differenceInCalendarISOWeeks(new Date(2017, 0 /* Jan */, 1), new Date(NaN));
    expect(isNaN(result)).toBe(true);
  });
