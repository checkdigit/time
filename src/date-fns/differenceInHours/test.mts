...
describe('differenceInHours', () => {
  ...
  it('returns the number of hours between the given dates', () => {
    const result = differenceInHours(new Date(2014, 6 /* Jul */, 2, 20, 0), new Date(2014, 6 /* Jul */, 2, 6, 0));
    expect(result).toBe(14);
  });

  it('returns a negative number if the time value of the first date is smaller', () => {
    const result = differenceInHours(new Date(2014, 6 /* Jul */, 2, 6, 0), new Date(2014, 6 /* Jul */, 2, 20, 0));
    expect(result).toBe(-14);
  });
  ...
  describe('edge cases', () => {
    it('the difference is less than an hour, but the given dates are in different calendar hours', () => {
      const result = differenceInHours(new Date(2014, 8 /* Sep */, 5, 12), new Date(2014, 8 /* Sep */, 5, 11, 59));
      expect(result).toBe(0);
    });

    it('the same for the swapped dates', () => {
      const result = differenceInHours(new Date(2014, 8 /* Sep */, 5, 11, 59), new Date(2014, 8 /* Sep */, 5, 12));
      expect(result).toBe(0);
    });

    it('the difference is an integral number of hours', () => {
      const result = differenceInHours(new Date(2014, 8 /* Sep */, 5, 13, 0), new Date(2014, 8 /* Sep */, 5, 12, 0));
      expect(result).toBe(1);
    });

    it('the given dates are the same', () => {
      const result = differenceInHours(new Date(2014, 8 /* Sep */, 5, 0, 0), new Date(2014, 8 /* Sep */, 5, 0, 0));
      expect(result).toBe(0);
    });
  ...
  it('returns NaN if the first date is `Invalid Date`', () => {
    const result = differenceInHours(new Date(NaN), new Date(2017, 0 /* Jan */, 1));
    expect(isNaN(result)).toBe(true);
  });

  it('returns NaN if the second date is `Invalid Date`', () => {
    const result = differenceInHours(new Date(2017, 0 /* Jan */, 1), new Date(NaN));
    expect(isNaN(result)).toBe(true);
  });
