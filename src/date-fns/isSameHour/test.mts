...
describe('isSameHour', () => {
  it('returns true if the given dates have the same hour', () => {
    const result = isSameHour(new Date(2014, 8 /* Sep */, 4, 6, 0), new Date(2014, 8 /* Sep */, 4, 6, 30));
    expect(result).toBe(true);
  });

  it('returns false if the given dates have different hours', () => {
    const result = isSameHour(new Date(2014, 8 /* Sep */, 4, 6, 0), new Date(2014, 8 /* Sep */, 4, 5, 0));
