...
describe('differenceInQuarters', () => {
  it('returns the number of full quarters between the given dates  with `trunc` as a default rounding method', () => {
    const result = differenceInQuarters(new Date(2012, 6 /* Jul */, 2, 5, 0), new Date(2011, 6 /* Jul */, 2, 6, 0));
    expect(result).toBe(3);
  });

  it('returns the number of full quarters between the given dates', () => {
    const result = differenceInQuarters(new Date(2012, 6 /* Jul */, 2, 18, 0), new Date(2011, 6 /* Jul */, 2, 6, 0));
    expect(result).toBe(4);
  });

  it('returns the number of full quarters between the given dates with `trunc` as a default rounding method', () => {
    const result = differenceInQuarters(new Date(2012, 6 /* Jul */, 2, 18, 0), new Date(2011, 4 /* May */, 2, 6, 0));
    expect(result).toBe(4);
  });
  ...
  it('returns a negative number if the time value of the first date is smaller', () => {
    const result = differenceInQuarters(new Date(2011, 6 /* Jul */, 2, 6, 0), new Date(2012, 6 /* Jul */, 2, 18, 0));
    expect(result).toBe(-4);
  });
  ...
  describe('edge cases', () => {
    it('the difference is less than a quarter, but the given dates are in different calendar quarters', () => {
      const result = differenceInQuarters(new Date(2014, 6 /* Jul */, 1), new Date(2014, 5 /* Jun */, 30));
      expect(result).toBe(0);
    });

    it('the same for the swapped dates', () => {
      const result = differenceInQuarters(new Date(2014, 5 /* Jun */, 30), new Date(2014, 6 /* Jul */, 1));
      expect(result).toBe(0);
    });

    it('the days of months of the given dates are the same', () => {
      const result = differenceInQuarters(new Date(2014, 3 /* Apr */, 6), new Date(2014, 0 /* Jan */, 6));
      expect(result).toBe(1);
    });

    it('the given dates are the same', () => {
      const result = differenceInQuarters(new Date(2014, 8 /* Sep */, 5, 0, 0), new Date(2014, 8 /* Sep */, 5, 0, 0));
      expect(result).toBe(0);
    });
  ...
  it('returns NaN if the first date is `Invalid Date`', () => {
    const result = differenceInQuarters(new Date(NaN), new Date(2017, 0 /* Jan */, 1));
    expect(isNaN(result)).toBe(true);
  });

  it('returns NaN if the second date is `Invalid Date`', () => {
    const result = differenceInQuarters(new Date(2017, 0 /* Jan */, 1), new Date(NaN));
    expect(isNaN(result)).toBe(true);
  });
