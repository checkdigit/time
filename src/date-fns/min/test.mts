...
describe('min', () => {
  it('returns the earliest date', () => {
    const result = min([new Date(1989, 6 /* Jul */, 10), new Date(1987, 1 /* Feb */, 11)]);
  ...
  it('accepts timestamps', () => {
    const result = min([new Date(1989, 6 /* Jul */, 10).getTime(), new Date(1987, 1 /* Feb */, 11).getTime()]);
    expect(result).toEqual(new Date(1987, 1 /* Feb */, 11));
  });

  it('returns `Invalid Date` if any given date is invalid', () => {
    const result = min([new Date(1989, 6 /* Jul */, 10), new Date(NaN), new Date(1987, 1 /* Feb */, 11)]);
