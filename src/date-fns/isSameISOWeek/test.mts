...
describe('isSameISOWeek', () => {
  it('returns true if the given dates have the same ISO week', () => {
    const result = isSameISOWeek(new Date(2014, 8 /* Sep */, 1), new Date(2014, 8 /* Sep */, 7));
    expect(result).toBe(true);
  });

  it('returns false if the given dates have different ISO weeks', () => {
    const result = isSameISOWeek(new Date(2014, 8 /* Sep */, 1), new Date(2014, 8 /* Sep */, 14));
    expect(result).toBe(false);
  });

  it('accepts a timestamp', () => {
    const result = isSameISOWeek(new Date(2014, 5 /* Jun */, 30).getTime(), new Date(2014, 6 /* Jul */, 2).getTime());
    expect(result).toBe(true);
  });
